...
- **Play in fullscreen** (auto-fullscreen mode). Turn it on, then pick where it applies under
  **Go fullscreen on**: all devices, mobile only, or desktop only. Mobile means phones and
  tablets, judged by the device, so a narrow desktop browser window still counts as desktop.
  The player uses the fullscreen type the video is set to, which is the immersive mode by default.
  Immersive fills the screen from its place in the page, so the viewer can still scroll down to your
  offer while the video plays. Turn on **Lock Scroll** under Immersive Mode to pin it and freeze the
  page instead.

**Impact on Integration:**
- Available controls affect which user interactions are possible
- Auto-fullscreen enters fullscreen when playback starts with sound and leaves it when the viewer
  pauses. A viewer who leaves fullscreen while it plays stays inline until they pause and play again. It does not fire a `fullscreen` page event. Native fullscreen fires `enter-fullscreen`
  and `exit-fullscreen`; the immersive mode fires neither.
...
- Auto-fullscreen works whether or not the fullscreen button is shown
