...
- **Event name** (any name you choose, for example "cta", "offer", "milestone")
...
<Warning>
**Important**: You must use the exact event name you configured in your dashboard settings. The examples below are illustrations. Replace them with your actual event names.
</Warning>
...
It fires as soon as a returning viewer starts watching with sound, not at the time you set, because they already saw that moment.
This is useful to show your CTA right away to people who come back.

The toggle needs **Call once** on the same event. That is what remembers the viewer, in a cookie, for 365 days.
...
**Event:** `cta_subsequent` (returning visitors, generated automatically)

**Data:**
```javascript
{
  time: 0.4,            // Where the returning viewer was when it fired (seconds), usually the start
  percentage: 0.2,      // Percentage of video completed
  session_id: "abc123"  // Device/session identifier
}
```
...
### A different time for returning visitors

`cta_subsequent` has no time of its own. To show the CTA to returning viewers at a specific point,
use it to know the viewer is back, then wait for that point with the `time` event:

```javascript
document.addEventListener('TrackPlayReady', function (e) {
  var player = e.detail.player;
  var RETURNING_AT = 3; // seconds into the video

  // First visit: fires at the time set in your dashboard
  player.on('cta', showCallToAction);

  // Returning visit: wait until RETURNING_AT, then show it once
  player.on('cta_subsequent', function () {
    function check(event) {
      if (event.detail.time >= RETURNING_AT) {
        player.off('time', check);
        showCallToAction();
      }
    }
    player.on('time', check);
  });
});
```

Actions you set on the event in the dashboard (show or hide elements, scroll, custom JavaScript) run when
`cta_subsequent` fires, so for a delayed returning-visitor CTA, do the showing in code like above.
...
### Call Frequency
- **Call once**: Event fires once per browser, then never again for **365 days**. It is
  remembered in a cookie, not in the session, so a returning viewer does not re-fire it.
...
5. **Use event data**: Read the time and percentage data for better tracking
