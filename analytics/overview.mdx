...
Broken out by affiliate, by referring domain, and by traffic source. See
[Sources](/analytics/sources) for the full report and how each session is classified.
...
- The **[Analytics API](/api-reference/analytics)** pulls summaries, retention curves, raw
  events and conversions, with a token carrying `analytics:read`.
- **Report exports** run as a job. You queue one with `POST /exports`, poll it, and get a
  download URL back when it is built.
- **[Audience exports](/audience/exports)** work differently. They stream a CSV straight
  back to you, with no job and no URL, and they are capped per download.
