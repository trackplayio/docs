...
### ElasticFunnels
- Checkout links matching `/b?p=` are tagged with the session on `vtid`
- ElasticFunnels returns it as `tracking.vtid` on every postback
- Sales, refunds, chargebacks and renewals all join back to the play
