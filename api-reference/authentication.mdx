...
Six scopes are enforced on live endpoints. Each one below names exactly what it unlocks.
...
<ParamField path="events:write" type="write-only">
  ...
  backend. This is the only scope the events host checks.
</ParamField>
...
<ParamField path="analytics:read" type="read-only">
  Read the [Analytics API](/api-reference/analytics): summary, retention, raw events and
  conversions, plus queueing and polling exports.
</ParamField>

<ParamField path="videos:read" type="read-only">
  List and read videos, their settings, their embed code and their player config. It also
  covers reading playlists **and workspace metadata**, which has no scope of its own.
</ParamField>

<ParamField path="videos:write" type="write-only">
  Create, update, duplicate and delete videos. Save a settings draft, deploy it, and
  restore a past version. Upload media. Replace captions, chapters, AI segments and CTA
  cards. Create and edit playlists.
</ParamField>

<ParamField path="webhooks:write" type="write-only">
  Manage webhook subscriptions, send test deliveries, and drive the Zapier REST hooks.
</ParamField>

### Reserved scopes

Three more scopes can be ticked when you mint a token: `conversions:read`,
`integrations:read`, and `workspace:read`.

<Warning>
  **No endpoint checks any of them today.** A token minted with only a reserved scope is
  accepted as a valid token and then rejected by every endpoint you point it at, because
  none of them is the scope that endpoint requires.

  They are reserved for read endpoints that do not exist yet. Do not build against them.
  To read workspace metadata today, use `videos:read`.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="503: AUTH_UNAVAILABLE (events host only)">
    ...
    This code is returned by the events host (`https://e.trackplay.io`). The app API on
    `https://app.trackplay.io` does not use it.
  </Accordion>
</AccordionGroup>
