...
<Note>
  ...
  `422`, and a `200` response that looks like it worked. `pitch_time_options` is the one
  group that behaves this way today, so it cannot be set through this API at all.
...
than rejected: only the ~35 known columns are read out of your request body.
