...
Over the limit returns `429`. Back off and retry: with your `event_id`, the retry is free.

The two limits answer differently, so key your handling off the `429` status, not off the
body. The per-workspace limit returns `{"error": "RATE_LIMITED", "message": "Too many
events for this workspace."}`. The per-IP limit returns `{"error": "Too many requests"}`
with no error code.
...
<AccordionGroup>
  ...
  <Accordion title="429: rate limited">
    Too many events for this workspace or IP. The workspace limit answers with
    `error: "RATE_LIMITED"`; the per-IP limit answers `{"error": "Too many requests"}` and
    carries no code, so match on the status.
  </Accordion>
