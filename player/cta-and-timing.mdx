...
<Steps>
  ...
  <Step title="Decide if it repeats">
    Leave **call once** on to fire at most once per browser. Note this is not per session:
    it writes a cookie that lasts **365 days**, so a viewer who comes back next week will
    not fire it again.
  </Step>
</Steps>
...
| **Scroll to element** | Smoothly scrolls the page to the first element matching a CSS selector, such as your order section. The video keeps playing. In immersive mode it scrolls without interrupting the takeover; with Lock Scroll on, or in native fullscreen, it leaves fullscreen first so the page can move. |
...
Actions run in this order: show, hide, JavaScript, then scroll. So one event can reveal a
hidden order section and scroll to it, with the same selector in both.
...
Your retention graph and funnel report both show a **Pitch** step, a marker for the
moment your offer actually lands. It comes from a mechanism separate from every timed
CTA or event on this page.

A timed event or CTA card *shows or hides something on the page*. Pitch Time is
different: it fires its own one-per-session `pitch-time-reached` event the moment
playback crosses a configured second, and TrackPlay's retention chart, funnel and lead
scoring are all built to read that event specifically. Setting timed events at the
second your pitch starts does not draw the Pitch marker. Only `pitch-time-reached` does.

Set it at the top of the **Timed Events** tab in Customize Player: **Pitch time**, in
hours, minutes and seconds. You rarely have to type it. TrackPlay reads your transcript,
finds the second your script turns to selling, and offers it as a one-click **Use**
button next to the field.

Until you set one, your reports already use that detected second: the retention graph
draws a **Pitch (detected)** line, and the funnel, the hot-lead score and the AI report
count "reached the offer" from it, by how far each viewer watched. Setting it makes the
player fire the real `pitch-time-reached` event on top of that, which you can listen for
on your page, and the retention graph then shows the measured median.
