...
<Warning>
  There is no tab for this on the Customize screen, and it is not reachable through the
  Video Settings API either: `pitch_time_options` is not one of the option groups that
  API accepts. Setting it today means asking TrackPlay to configure it for your video.
  Once it is set, the Pitch marker appears on the retention graph and the funnel's
  `pitch_reached` step starts counting, with no further action from you.
</Warning>
